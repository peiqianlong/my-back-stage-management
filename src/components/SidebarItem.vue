      ...
      <ul class="nav">
        <template v-for="item in routes">
            <router-link tag="li"  class="nav-item nav-dropdown"  v-if="!item.hidden&&item.children&&item.children.length>0" :to="item.path+''+item.children[0].path" disabled>
                ...
                <div class="nav-link nav-dropdown-toggle" @click="handleClick"><Icon size="22" :type="item.icon" color="white"/>{{ item.name}}  </div>
        ...
        <li class="nav-item" v-if="!item.hidden&&!item.children">
          <router-link  :to="item.path" class="nav-link" exact><Icon size="22" :type="item.icon" color="white" />{{ item.name}} </router-link>
        </li>
